...
const IconSearch  = () => <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="#94a3b8" strokeWidth="2" strokeLinecap="round"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg>;
  ...
  const submit = async e => {
    ...
    try {
      ...
      if (isEdit) {
        ...
        if (initial_note?.trim() && res.data.data?.id) {
        ...
        <form onSubmit={submit} style={{ display:'flex', flexDirection:'column', gap:'14px' }}>
          ...
          <div style={styles.row2}>
            <div>
              ...
              <select name="lead_source" value={form.lead_source || 'website'} onChange={handle} style={inputStyle}>
                {SOURCES.map(s => <option key={s} value={s}>{s.replace('_',' ')}</option>)}
              </select>
            </div>
            <div>
              <label style={labelStyle}>Status</label>
              <select name="status" value={form.status || 'new'} onChange={handle} style={inputStyle}>
...
export default function LeadsPage() {
  ...
  const filtered = leads.filter(l => {
    ...
    if (filterAssigned === 'me' && typeof window !== 'undefined') {
      try {
        const token = localStorage.getItem('token');
        if (token) {
          const payload = JSON.parse(atob(token.split('.')[1]));
          matchAssigned = l.assigned_to === payload.id;
        }
      } catch (e) {}
    }
        ...
        <div style={styles.body}>
          ...
          {loading ? (
            ...
            <div style={styles.empty}>
              <svg width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="#cbd5e1" strokeWidth="1.5" strokeLinecap="round"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/></svg>
              <p style={{ color:'#64748b', fontSize:'15px', margin:'12px 0 4px', fontWeight:600 }}>No leads found</p>
              <p style={{ color:'#94a3b8', fontSize:'13px' }}>Add your first lead to get started</p>
            </div>
          ) : (
                    ...
                    <tr key={lead.id} style={{ ...styles.tr, animationDelay: `${i * 40}ms` }} className="lead-row">
                      ...
                      <td style={styles.td}>
                        <div style={styles.actions}>
                          <button onClick={() => router.push(`/leads/${lead.id}`)} style={styles.iconBtn} title="View"><IconEye /></button>
                          <button onClick={() => setModal(lead)} style={styles.iconBtn} title="Edit"><IconEdit /></button>
                          <button onClick={() => setDeleteId(lead.id)} style={{ ...styles.iconBtn, color:'#ef4444' }} title="Delete"><IconTrash /></button>
                        </div>
                      </td>
                    </tr>
                  ))}
...
const styles = {
  ...
  searchInput:{ flex:1, background:'transparent', border:'none', color:'#0f172a', fontSize:'14px', fontFamily:"'Inter',sans-serif", outline:'none' },
  ...
  spinner:    { width:'36px', height:'36px', border:'3px solid #e2e8f0', borderTop:'3px solid #3b82f6', borderRadius:'50%' },
