            ...
            <div style={S.panel}>
              ...
              {report && tab === 'pipeline' && !loading && (
                <div style={{ display:'flex', flexDirection:'column', gap:'18px' }}>
                  <div style={{ display:'flex', alignItems:'center', gap:'20px' }}>
                    <div style={{ ...S.scoreRing, borderColor: report.healthScore >= 70 ? '#059669' : report.healthScore >= 40 ? '#d97706' : '#dc2626' }}>
                      <span style={{ fontSize:'26px', fontWeight:800, color: report.healthScore >= 70 ? '#059669' : report.healthScore >= 40 ? '#d97706' : '#dc2626' }}>{report.healthScore}</span>
              ...
              {!result && !report && !loading && (
                <div style={S.emptyOut}>
                  <svg width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="#cbd5e1" strokeWidth="1.5" strokeLinecap="round"><rect x="3" y="11" width="18" height="10" rx="2"/><path d="M12 11V7"/><circle cx="12" cy="5" r="2"/></svg>
                  <p style={{ color:'#94a3b8', fontSize:'14px', marginTop:'12px' }}>Configure your request on the left and click run</p>
                </div>
              )}
            </div>
          </div>
...
const S = {
  ...
  bigSpinner:  { width:'40px', height:'40px', border:'3px solid #e2e8f0', borderTop:'3px solid #3b82f6', borderRadius:'50%' },
